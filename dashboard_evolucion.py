...
try:
    ...
    # NORMALIZADOR INTELIGENTE DE COLUMNAS (Elimina tildes y espacios ocultos de Google Forms)
    df.columns = df.columns.str.strip().str.replace('Í', 'I').str.replace('í', 'i').str.replace('á', 'a').str.replace('é', 'e').str.replace('ó', 'o').str.replace('ú', 'u')
    
    # Traducir automáticamente la columna de tiempo a fechas reales
    col_fecha = 'Fecha' if 'Fecha' in df.columns else df.columns[0]
    df[col_fecha] = pd.to_datetime(df[col_fecha], errors='coerce')
    
    # Extraer el mes en español de forma automática
    meses_es = {1: 'enero', 2: 'febrero', 3: 'marzo', 4: 'abril', 5: 'mayo', 6: 'junio',
                7: 'julio', 8: 'agosto', 9: 'septiembre', 10: 'octubre', 11: 'noviembre', 12: 'diciembre'}
    df['Mes'] = df[col_fecha].dt.month.map(meses_es).fillna('desconocido')
    
    # Asegurar que el Monto sea procesado estrictamente como número
